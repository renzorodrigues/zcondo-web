  ...
  <CalendarRoot v-slot="{ date, grid, weekDays }" v-model:placeholder="placeholder" v-bind="forwarded"
    ...
    <CalendarHeader>
      <CalendarHeading class="flex w-full items-center justify-between gap-2">
        <Select :default-value="placeholder.month.toString()" @update:model-value="(v: string) => {
        ...
        <Select :default-value="placeholder.year.toString()" @update:model-value="(v: string) => {
          if (!v || !placeholder) return;
          ...
          <SelectContent class="max-h-[200px]">
            <SelectItem
              v-for="year in Array.from({ length: (new Date().getFullYear() - 15) - (new Date().getFullYear() - 90) + 1 }, (_, i) => new Date().getFullYear() - 90 + i).reverse()"
